 ...
 */
   if (!l2size)
      l2size = ((ptrbits == 64) ? 32 : 4) * 1024*1024;
   else
      l2size *= 1024;
   if (!f2cdefs) f2cdefs = "";
/*
 ...
 */
   ...
   else if (OS == OSWin64 && ptrbits == 64)
      fprintf(fpout, "   ADdir  = $(SRCdir)/CONFIG/ARCHS/WIN64\n");
   else
 ...
 */
   if (strstr(comps[NCOMP+DKC_], " -m32"))
      ...
      fprintf(fpout, " -m64");
   else if (strstr(comps[NCOMP+DKC_], " -maix64"))
      fprintf(fpout, " -maix64");
   else if (strstr(comps[NCOMP+DKC_], " -maix32"))
      fprintf(fpout, " -maix32");
